...
namespace FlowWordsNiftory
{
    public class FlowInterface : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            // Register Fcl
            IWallet linkingWalletProvider = new FclProvider();
            linkingWalletProvider.Init(new FclConfig
            {
                Description = "A word game on Flow",
                IconUri = "https://walletconnect.com/meta/favicon.ico",
                Location = "",
                Title = "Flow Words",
                Url = "https://dapperlabs.com",
                WalletConnectProjectId = "fb087df84af28bc20669151a5efb3ff7" // insert Project ID from Wallet Connect dashboard
            });

            FlowSDK.RegisterWalletProvider(walletProvider, linkingWalletProvider);
        }
        ...
        public IEnumerator GetGameDataFromChain(string username, System.Action<Decimal, List<GuessResult>, Dictionary<string, string>> onSuccessCallback, System.Action onFailureCallback)
        {
            ...
            string txnId = null;
            Task<FlowTransactionResult> getStateTask = Transactions.SubmitAndWaitUntilExecuted((string id) => { txnId = id; Debug.Log($"txnId: {txnId}"); }, DoTextSubstitutions(loginTxn.text), new CadenceString(username));
            ...
            if (getStateTask.Result.Error != null)
            {
                Debug.LogError($"Error getting transaction result: {getStateTask.Result.Error.Message}");
                onFailureCallback();
                yield break;
            }

            if (getStateTask.Result.ErrorMessage != string.Empty)
            {
                Debug.LogError($"Transaction execution error: {getStateTask.Result.ErrorMessage}");
                onFailureCallback();
                yield break;
            }

            if (getStateTask.Result.Status == FlowTransactionStatus.EXPIRED)
            {
                Debug.LogError("Transaction expired.");
                onFailureCallback();
                yield break;
            }
        ...
        {
            IWallet walletProvider = FlowSDK.GetWalletProvider();
            IWalletlessOnboarding walletlessOnboarding = walletProvider as IWalletlessOnboarding;

            if (walletlessOnboarding == null)
            {
                Debug.LogError("LinkAccountClicked: The registered wallet provider does not implement IWalletlessOnboarding, and therefore cannot be used for account linking.");
                return;
            }
